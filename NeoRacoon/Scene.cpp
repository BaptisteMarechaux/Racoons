...
void Scene::GeometryPass ( )
{
	...
	glDrawArrays ( GL_LINES , 0 , originShapeVertices.size ( ) );
...
void Scene::AddPointOriginShapeVertices ( Surface3D surf , glm::vec3 position )
{
	...
	{
		originShapeVertices.push_back ( surf.get_Edges ( ) [ i ]->get_A ( ) );
		originShapeVertices.push_back ( surf.get_Edges ( ) [ i ]->get_B ( ) );
	}
	/*if ( !surf.get_Close ( ) )
	{
...
void Scene::AddPointVertices ( Surface3D surf , glm::vec3 position )
{
	for ( int i = 0; i < surf.get_Edges ( ).size ( ); ++i )
	{
		...
		vertices.push_back ( surf.get_Edges ( ) [ i ]->get_B ( ) );
	}
	/*if ( !surf.get_Close ( ) )
	{
	//vertices.push_back ( surf.get_Edges ( ) [ surf.get_Edges ( ).size ( ) - 1 ]->get_B ( ) );
